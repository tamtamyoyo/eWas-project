...
export default function InstagramCallback() {
  ...
  useEffect(() => {
    const completeInstagramAuth = async () => {
      try {
        ...
        if (!code) {
          setError(t("connect.instagram.invalidCallback", "Invalid callback parameters"));
          setIsProcessing(false);
          return;
        }

        console.log("Instagram callback received with code, sending to server");
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || "Failed to complete Instagram authentication");
        }

        // Refresh user authentication status
        console.log("Instagram connection successful, refreshing auth state");
        ...
        setError(error.message || t("connect.instagram.callbackError", "Failed to process authentication callback"));
        setIsProcessing(false);
        
        toast({
          title: t("connect.errorTitle", "Error"),
          description: error.message || t("connect.instagram.callbackError", "Failed to process authentication callback"),
          variant: "destructive",
        });
        ...
        <div className="mb-6">
          {isProcessing ? (
            <>
              ...
              <p className="text-neutral-500">{t("connect.instagram.completingAuth", "Completing Instagram authentication...")}</p>
            </>
          ) : (
            <>
              <div className="mb-4 flex justify-center">
              ...
              <p className="mt-4 text-sm">
                {t("connect.instagram.redirecting", "Redirecting you back to the connect page...")}
              </p>
            </>
          )}
        </div>
